...
page_title: "etleap_connection_gitlab Resource - terraform-provider-etleap"
subcategory: ""
description: |-
  ConnectionGITLAB Resource
---

# etleap_connection_gitlab (Resource)
...
- `access_token` (String) Your Personal Access Token can be generated in your account by clicking on your avatar > edit profile > Access Tokens on the left sidebar. Etleap will need the read_user and read_api scopes.
- `name` (String) The unique name of this connection.
- `type` (String) must be one of ["GITLAB"]

### Optional

- `api_url` (String) Your Gitlab instance URL if you're on a self hosted setup. Leave empty for gitlab.com. Example: https://gitlab.example.com. Requires replacement if changed.
