...
func (r *ConnectionREDSHIFTResourceModel) ToSharedConnectionRedshiftInput() *shared.ConnectionRedshiftInput {
	name := r.Name.ValueString()
	typeVar := shared.ConnectionRedshiftType(r.Type.ValueString())
...
func (r *ConnectionREDSHIFTResourceModel) RefreshFromSharedConnectionRedshift(resp *shared.ConnectionRedshift) {
	r.Active = types.BoolValue(resp.Active)
	r.Address = types.StringValue(resp.Address)
	r.CreateDate = types.StringValue(resp.CreateDate.Format(time.RFC3339Nano))
	r.Database = types.StringValue(resp.Database)
	r.DataSharingDestinations = nil
	for _, v := range resp.DataSharingDestinations {
		r.DataSharingDestinations = append(r.DataSharingDestinations, types.StringValue(v))
	}
	...
	r.DynamicVarcharWidthEnabled = types.BoolPointerValue(resp.DynamicVarcharWidthEnabled)
	r.ID = types.StringValue(resp.ID)
	r.Name = types.StringValue(resp.Name)
	r.Port = types.Int64Value(resp.Port)
	r.QueryTagsEnabled = types.BoolPointerValue(resp.QueryTagsEnabled)
	...
	for _, v := range resp.UserGroups {
		r.UserGroups = append(r.UserGroups, types.StringValue(v))
	}
	r.Username = types.StringValue(resp.Username)
}

func (r *ConnectionREDSHIFTResourceModel) ToSharedConnectionRedshiftUpdate() *shared.ConnectionRedshiftUpdate {
	name := new(string)
	...
	typeVar := shared.ConnectionRedshiftUpdateType(r.Type.ValueString())
	active := new(bool)
	...
	var sshConfig *shared.ConnectionRedshiftUpdateSSHConfigurationUpdate
	if r.SSHConfig != nil {
		address1 := new(string)
...
func (r *ConnectionREDSHIFTResourceModel) ToSharedConnectionDelete() *shared.ConnectionDelete {
